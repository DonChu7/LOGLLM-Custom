...
if __name__ == '__main__':
    ...
    # --- TinyLlama safety tweak: make sure pad_token exists ---
    if getattr(model, "Llama_tokenizer", None) is not None:
        tok = model.Llama_tokenizer
        if tok.pad_token is None:
            tok.pad_token = tok.eos_token
            tok.padding_side = "left"     # left padding is typical for decoder-only models
    # ----------------------------------------------------------
    ...
    dataloader_max_samples = DataLoader(
        ...
        num_workers=0,
        sampler=BalancedSampler(dataset, target_ratio=min_less_portion, max_samples=max(1, min(1000, len(dataset) - 1))),
        collate_fn=collator,
        drop_last=True
    )
    # phase 1
